$(document).ready(function () {
  ...
  axios
    .get("/api/categories")
    .then(rsp => {
      var categories = rsp.data.cats.categories;
        ...
        if (!category.unlisted) {
          ...
          $("#categoryNav").append(
            `<a href="#${category.title
              ...
              .replace(
                /\s/g,
                ""
              )}" class="border p-2 mt-4 mb-2 mx-2 text-white shadow-sm border-radius-8 categoryButton">${
              category.title
            }</a>`
          );
          $("#products").append(
            `
                    <div class="row mt-4">
                        <div class="col-lg-12">
                            ...
                            <p>Grab these premium accounts at dirt cheap prices.</p>
                            </div>
                        </div>
                    </div>
                `
          ...
          $("#products").append(
            '<div class="row justify-content-center" id="' +
              category.title.replace(/[^a-zA-Z ]/g, "").replace(/\s/g, "") +
              '"></div>'
          );
              ...
              `
                    <div class="col-lg-4 col-md-6">
                            <div class="feature_box">
                                <img
                                    src="https://cdn.sellix.io/static/images/products/${
                                      product.image_attachment + ".png"
                                    }"
                                    alt="next"
                                    class="img-fluid"
                                />
                                <div class="d_flex">
                                    <div class="feat_head_flx">
                                        <h6>${product.title}</h6>
                                        <p>Stock : ${product.stock}</p>
                                    </div>
                                    <div class="feat_head_flx">
                                        <h6>£${product.price_display}</h6>
                                        <a href="https://sellix.io/product/${
                                          product.uniqid
                                        }">View More</a>
                                    </div>
                                </div>

                                <button type="button" class="btn btn_custom" data-sellix-product="${
                                  product.uniqid
                                }">
                                    PURCHASE
                                </button>
  ...
  axios.get("/api/feedback").then(rsp => {
    var feedbacks = rsp.data.fb.feedback.reverse();
    $("#totalFeedbacks").text(feedbacks.length + 13 + " Positive Feedback On");

    var i = 0;
    feedbacks.forEach(feedback => {
      if (i < 3) {
        let f = feedbacks[Math.floor(Math.random() * feedbacks.length)];
        $("#feedback1").append(
          `<div class="col-md-4 mb-3">
                  <a href="https://sellix.dark.shop/feedback" target="_blank">
                  ...
                  ${
                    f.star === 0
                      ? ""
                      : f.star === 1
                      ? `<span class="fa fa-star star-green"></span>`
                      : f.star === 2
                      ? `<span class="fa fa-star star-green"></span>
                    <span class="fa fa-star star-green"></span>`
                      : f.star === 3
...
function getRating(stars) {
  ...
  for (var i = 0; i < stars; i++) {
    star = +'<span class="fa fa-star star-green"></span>';
  }
  return star;
}
